...
단독 테스트에서 `memory-list.js`는 p(95) 38ms였지만, 홈에서는 221ms로 올라갔다. HikariCP 풀 메트릭 등을 직접 측정하지는 않았지만, 3개 API가 동시에 DB 커넥션과 스레드를 경합하면서 발생한 증가로 추정된다. 홈 체감 로딩 속도는 가장 느린 `/api/memory` 응답이 결정한다. 그래도 목표치(500ms) 대비 충분한 여유가 있다.
