...
### 멤버 초대
- UUID 기반 초대 링크 생성 (만료 시각 / 최대 사용 횟수 설정)
...
* **성능 SLA (SLA Goal):** 50 VUs(가상 사용자) × 30초 부하 조건 하에 **p(95) 미만 500ms**, **에러율 미만 1%** 방어 기준 수립 (Google RAIL 모델 벤치마킹).
...
| 테스트 시나리오 | 대상 엔드포인트 | p(95) 지표 | 에러율 | 결과 및 분석 (Analysis) |
|---|---|---|---|---|
