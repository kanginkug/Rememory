...
public class MemoryController {
    ...
    @PutMapping
    public ResponseEntity<Void> updateMemory(@RequestAttribute("memberId") Long memberId, @RequestBody @Valid UpdateMemoryRequestDTO umRequestDTO) {
        memoryService.updateMemory(memberId, umRequestDTO);
        return ResponseEntity.ok().build();
    }

    @GetMapping
    public ResponseEntity<List<Memory>> findMemoryList(@RequestAttribute("memberId") Long memberId, @RequestParam("sortType") SortTypeMemory sortTypeMemory, @RequestParam(value = "keyword", required = false) String keyword) {
    ...
    public ResponseEntity<Void> updateMemoryPhoto(
        ...
        String imageUrl = memoryService.madePhotoUrl(file);

        // 2. 비즈니스 로직을 서비스 하나로 묶어서 처리
        // 파일 URL 생성 + DB 업데이트를 서비스 안에서 한 번에 해결합니다.
        memoryService.updateMemoryPhoto(memoryId, memberId, imageUrl);
